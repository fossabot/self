...
function bumpBuild(platform = 'both') {
  ...
  if (platform === 'android' || platform === 'both') {
    ...
    console.log(`✅ Android build number bumped to ${versionData.android.build}`);
  }
...
switch (command) {
  ...
  default:
    console.log('Usage:');
    console.log('  node version.cjs bump-build [ios|android|both]  - Bump build number');
    console.log('  node version.cjs deployed [ios|android|both]     - Update deployment timestamp');
    console.log('  node version.cjs info                            - Get current version info');
    console.log('');
