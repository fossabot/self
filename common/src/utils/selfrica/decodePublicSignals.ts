...
export const decodePublicSignals = (publicSignals: string[]) => {
  const revealedDataPacked = publicSignals.slice(
    SELFRICA_PUBLIC_SIGNALS_REVEALED_DATA_PACKED,
    SELFRICA_PUBLIC_SIGNALS_REVEALED_DATA_PACKED +
      SELFRICA_PUBLIC_SIGNALS_REVEALED_DATA_PACKED_LENGTH
  );
  const revealedDataUnpacked = unpackReveal(revealedDataPacked, 'id').slice(
    0,
    SELFRICA_MAX_LENGTH + 2 + 3
  );

  const forbiddenCountriesPacked = publicSignals.slice(
    SELFRICA_PUBLIC_SIGNALS_FORBIDDEN_COUNTRIES_PACKED,
    SELFRICA_PUBLIC_SIGNALS_FORBIDDEN_COUNTRIES_PACKED +
      SELFRICA_PUBLIC_SIGNALS_FORBIDDEN_COUNTRIES_PACKED_LENGTH
  );
  ...
  const currentDate = publicSignals.slice(
    SELFRICA_PUBLIC_SIGNALS_CURRENT_DATE,
    SELFRICA_PUBLIC_SIGNALS_CURRENT_DATE + SELFRICA_PUBLIC_SIGNALS_CURRENT_DATE_LENGTH
  );
