...
import { groth16, PublicSignals } from 'snarkjs';
...
} from '../../common/src/mock_certificates/aadhaar/mockAadhaarCert.ts';
import {
  prepareAadhaarDiscloseData,
  prepareAadhaarRegisterData,
  processQRData,
} from '../../common/dist/esm/src/utils/aadhaar/mockData';
...
async function registerMockPassportOrEUid(
  ...
  if (ispassport) {
    ...
    passportData = genMockIdDocAndInitDataParsing({
      ...
      signatureType: `rsa_sha256_65537_4096`,
    });
  ...
  const proofType = registerCircuitName.startsWith('register_id_')
    ? 'register_id'
    : 'register';
...
export async function setupTestData(attestationId: string): Promise<void> {
  const secret = '1234';
...
export async function setupTestDataAadhaar() {
  ...
  const inputs = await prepareAadhaarRegisterData(qrdata.testQRData, '1234', [
    publicKeyPem,
  ]);
  ...
  const wasmPath = path.resolve(
    __dirnameHelper,
    'ts-api/utils/assests/vc_and_disclose_aadhaar.wasm',
  );
  const zkeyPath = path.resolve(
    __dirnameHelper,
    'ts-api/utils/assests/vc_and_disclose_aadhaar.zkey',
  );
  const vKey = JSON.parse(
    fs.readFileSync(
      path.resolve(
        __dirnameHelper,
        'ts-api/utils/assests/verification_key_aadhaar.json',
      ),
      'utf8',
    ),
  );
